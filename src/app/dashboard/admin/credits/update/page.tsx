  ...
  const fetchData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const userData = data.users || data.topUsers || [];
        console.log('Setting users with:', userData);
        setUsers(userData);
        ...
        if (userData.length === 0) {
          console.log('No users found with activity in this period');
        }
      } else {
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            {users.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-gray-500">No users found with content activity in this period</p>
                    ...
                    {users
                      ...
                      .map((user) => (
                        <tr key={user.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="text-right py-3">{user.activity?.captions || 0}</td>
                          <td className="text-right py-3">{user.activity?.titles || 0}</td>
                          <td className="text-right py-3">{user.activity?.videoScripts || 0}</td>
                          <td className="text-right py-3">{user.activity?.thumbnails || 0}</td>
                          <td className="text-right py-3">{user.activity?.socialPosts || 0}</td>
                          <td className="text-right py-3">{user.activity?.enhancedPosts || 0}</td>
                          <td className="text-right py-3 font-medium text-purple-600">{user.activity?.total || 0}</td>
                          <td className="text-right py-3">
                            <button
        ...
        {activeTab === 'content' && (
          <div className="bg-white rounded-lg shadow p-6">
            ...
            {recentContent.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-gray-500">No content found in this period</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="min-w-full">
